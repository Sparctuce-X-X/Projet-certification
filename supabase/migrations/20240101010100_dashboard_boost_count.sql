-- ─────────────────────────────────────────────────────────────────────────────
-- Migration 61 — Ajout `boosted` au breakdown annonces du dashboard vendeur
--
-- Suite à F09 (boost annonces), la RPC `get_my_dashboard_stats` (mig 58)
-- doit aussi remonter le nombre d'annonces actuellement boostées (boost
-- actif = is_boosted=true ET boost_until > now()).
--
-- Permet à l'UI dashboard d'afficher "✨ X boostée(s)" sur la KPI Annonces.
--
