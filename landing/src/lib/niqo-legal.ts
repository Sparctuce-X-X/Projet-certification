// Mirror Node/Next.js de `supabase/functions/_shared/niqo-legal.ts`.
//
// Le fichier Deno original utilise des imports ESM (`https://esm.sh/…`) et
// `Deno.env.get` — incompatibles avec le runtime Node de Next.js. Ce mirror
// duplique UNIQUEMENT les constantes et helpers de formatage (pas d'env, pas
// d'imports externes). À garder synchro manuellement avec le fichier source.
//
// Source de vérité : certificat RDB Rwanda (`docs/certificate-NIQO-LTD.pdf`,
// RDB serial 261170857304440, printing date 2026-04-27).
//
// ⚠ Toute modification des constantes légales (TIN, adresse, capital, etc.)
// doit être répercutée dans les DEUX fichiers en cohérence.
...
export function getNiqoLegalFooterHtml(): string {
  return `<p style="margin:0;font-size:12px;line-height:1.5;color:#5A5A57;font-family:Arial,sans-serif;">
